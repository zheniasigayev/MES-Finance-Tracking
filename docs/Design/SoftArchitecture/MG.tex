...
The use hierarchy instantiates the three-level decomposition summarized in Table~\ref{TblMH}: M1 abstracts the hardware boundary, software decision modules (M6--M8) publish cohesive services above that boundary, and behaviour-hiding modules (M2--M5) rely exclusively on those services. Because every path in the graph terminates at M1, any prefix of the hierarchy forms a testable, incrementally deployable subset of the platform and respects the DAG criterion from \citet{Parnas1978}.
...
\bottomrule
\end{tabular}
\caption{Summary of the use relationships among modules}
\label{TblUseSummary}
\end{table}
